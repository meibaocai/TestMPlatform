...
BROKER_URL = 'amqp://yoho:yoho@192.168.104.244:5672/yoho'
...
CELERYBEAT_SCHEDULE = {
    ...
    'run_run_Init_GlobalParameter': {
        'task': 'mycelery.runcase.tasks.run_Init_GlobalParameter',
        # 定时任务一：　每5分钟执行一次任务(refresh1)
        # 'schedule': crontab(hour='*/5'),
        # 定时任务二:　每天的凌晨2:00，执行任务(refresh2)
        'schedule': crontab(minute=0, hour=2),
        # 定时任务三:每个月的１号的6:00启动，执行任务(refresh3)
        # 'schedule': crontab(hour=6, minute=0, day_of_month='1'),
        #  pid:项目id,  pid, search_key:搜索关键字 , select_env：选择环境
        'args': (25, '', '')
    }
}
